...
export default function Hero() {
  return (
    <section className="relative min-h-[92vh] overflow-hidden" id="home">
      ...
      {/* Cinematic hero image with slow zoom-out */}
      <motion.img
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto pt-40 md:pt-48 px-6">
        ...
        >
          {["Explore", "Our Process", "Contact"].map((label, i) => (
            <motion.button
              key={label}
              variants={{ hidden: { y: 10, opacity: 0 }, show: { y: 0, opacity: 1 } }}
              className="rounded-2xl px-5 py-3 backdrop-blur-xl border border-white/15 bg-white/10 text-white hover:bg-white/20 transition-transform active:scale-[0.98]"
            >
              {label}
            </motion.button>
          ))}
